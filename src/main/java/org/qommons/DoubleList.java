/*
 * DoubleList.java Created Aug 10, 2010 by Andrew Butler, PSL
 */
package org.qommons;

/**
 * <p>
 * Acts like an {@link java.util.ArrayList} but for primitive double values.
 * </p>
 *
 * <p>
 * This class also has a sorted option that will cause the list to sort itself and maintain the sorted order (ascending). If this option is
 * set, calls to {@link #add(int, double)} will disregard the index parameter to maintain correct order. Calls to {@link #set(int, double)}
 * will behave the same as successive calls to {@link #remove(int)} and {@link #add(double)}.
 * </p>
 *
 * <p>
 * This class also has a unique option that will cause the list to contain at most one instance of any value. If this option is set, calls
 * to {@link #add(double)}, {@link #add(int, double)}, {@link #addAll(double...)}, etc. will not always add the values given if the value(s)
 * already exist. The unique feature has better performance if used with the sorted feature, but both features may be used independently.
 * </p>
 *
 * <p>
 * This class is NOT thread-safe. If an instance of this class is accessed by multiple threads, it MUST be synchronized externally.
 * </p>
 */
public class DoubleList implements Iterable<Double>, Sealable, Cloneable
{
	...
	/**
	 * Creates a double list with the option of having the list sorted and/or unique-constrained initially
	 *
	 * @param sorted Whether the list should be sorted
	 * @param unique Whether the list should eliminate duplicate values
	 */
	public DoubleList(boolean sorted, boolean unique)
	{
		this(5);
		isSorted = sorted;
		isUnique = unique;
	 ...
	 * @return The index at which the given value would be added into this array from an {@link #add(double) add} operation.
	 * @throws IllegalStateException If this list is not sorted
	 */
	public int indexFor(double value)
	{
		if(!isSorted)
	 ...
	 */
	public int addAll(double... value)
	{
	 ...
	 * @return The number of values added to this list
	 */
	public int addAll(double[] value, int start, int end, int insert)
	{
		assertUnsealed();
	 ...
	 * @return The number of values added to this list
	 */
	public int addAll(DoubleList list, int insert)
	{
	...
	/**
	 * Compares two doubles for equality. Interprets all Double.NaN values as equal and interprets 0.0 and -0.0 as different.
	 *
	 * @param f1 The first double to compare
	 * @param f2 The second double to compare
	 * @return Whether the two double values are equivalent
	 */
	 ...
	 */
	public int removeAll(DoubleList list)
	{
		assertUnsealed();
	...
	public void swap(int idx1, int idx2)
	{
		...
		double temp = theValue[idx1];
		theValue[idx1] = theValue[idx2];
		theValue[idx2] = temp;
	}

	@Override
	public java.util.ListIterator<Double> iterator()
	{
		return new DoubleListIterator(toArray());
	}
	 ...
	 */
	public int and(DoubleList list)
	{
		assertUnsealed();
		int ret = 0;
	 ...
	 */
	public int instanceCount(double value)
	{
		if(isSorted)
		{
	...
	/**
	 * Similar to {@link #toArray()} but creates an array of {@link Double} wrappers
	 *
	 * @return The list of values currently in this list
	 */
	public Double[] toObjectArray()
	{
		...
		for(int i = 0; i < ret.length; i++)
			ret[i] = Double.valueOf(theValue[i]);
		return ret;
	}
	...
	public void trimToSize()
	{
		if(theValue.length == theSize)
			return;
		double[] oldData = theValue;
		theValue = new double[theSize];
	...
	public void ensureCapacity(int minCapacity)
	{
		...
		if(minCapacity > oldCapacity)
		{
			...
			theValue = new double[newCapacity];
			System.arraycopy(oldData, 0, theValue, 0, theSize);
		}
	}

	private class DoubleListIterator implements java.util.ListIterator<Double>
	{
		private double[] theContent;

		private int theIndex;

		private boolean lastRemoved;

		DoubleListIterator(double[] content)
		{
			theContent = content;
		}
		...
		public Double next()
		{
			Double ret = Double.valueOf(theContent[theIndex]);
			theIndex++;
			lastRemoved = false;
			return ret;
		}
		...
		{
			Double ret = Double.valueOf(theContent[theIndex - 1]);
			theIndex--;
			lastRemoved = false;
			return ret;
		}
		...
		public void remove()
		{
			...
			DoubleList.this.remove(theIndex);
			double[] newContent = new double[theContent.length - 1];
		...
		public void set(Double e)
		{
			...
			if(get(theIndex) != theContent[theIndex])
				throw new java.util.ConcurrentModificationException(
					"List has been modified apart from this iterator");
			theContent[theIndex] = e.doubleValue();
			DoubleList.this.set(theIndex, e.doubleValue());
		}
